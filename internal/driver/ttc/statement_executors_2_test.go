...
func TestPrepareBindsAndOAC_Encode_Supported(t *testing.T) {
	...
	if !bytes.Equal([]byte(row[0].data), []byte(expNum)) {
		t.Fatalf("int64 encoding mismatch: got %v want %v", []byte(row[0].data), []byte(expNum))
	}
	expVcs, _ := converters.EncodeVarchar("X")
	if !bytes.Equal([]byte(row[1].data), []byte(expVcs)) {
		t.Fatalf("varchar encoding mismatch: got %v want %v", []byte(row[1].data), []byte(expVcs))
	}
	if !bytes.Equal([]byte(row[2].data), []byte{1, 2, 3}) {
		t.Fatalf("raw encoding mismatch: got %v want %v", []byte(row[2].data), []byte{1, 2, 3})
	}
	if row[3].data != nil {
		t.Fatalf("nil bind should encode as nil cell, got %v", row[3])
	}
	// bool -> representation per current impl
	if !bytes.Equal([]byte(row[4].data), []byte{1, 1}) {
		t.Fatalf("bool(true) encoding mismatch: got %v want %v", []byte(row[4].data), []byte{1})
	}
	if !bytes.Equal([]byte(row[5].data), []byte{0}) {
		t.Fatalf("bool(false) encoding mismatch: got %v want %v", []byte(row[5].data), []byte{0})
	}
	expTS, _ := converters.EncodeTimestampWithTimeZone(ts)
	if !bytes.Equal([]byte(row[6].data), []byte(expTS)) {
		t.Fatalf("timestamp encoding mismatch: got %v want %v", []byte(row[6].data), []byte(expTS))
	}
}
