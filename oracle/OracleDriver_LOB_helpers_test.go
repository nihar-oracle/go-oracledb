...
// createLOBObjectName assigns each top-level LOB scenario a distinct,
// recognizable Oracle object name. createObjectName supplies the run-unique
// random suffix; the normalized Go test name prevents logical-name overlap
// between independently runnable LOB tests.
func createLOBObjectName(t *testing.T) string {
	t.Helper()
